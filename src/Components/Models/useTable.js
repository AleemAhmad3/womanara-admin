...
export function useTable({ attributes, tableType, limitPerPage = 25 }) {
  ...
  if (tableType === "Products") {
    ...
    } else if (tableType === "SubscribedUsers") {
      response = await fetchallSubscribedUsers(page, rowsPerPage);
      setData(response.subscribers);
      setTotalRecords(response.totalSubscribers);
  ...
  const handleDelete = async () => {
    ...
    try {
     ...
     if (tableType === "Lead") {
        ...
        response = await deleteAllSubscribedUsers({ ids: selected });
        fetchData();
      }
       else {
